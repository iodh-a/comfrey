...
- **运行命令**：同 §二案例 1 的 `run_real_apps_experiment.py` 命令。  
...
## 四、错误类型三：Harness 检测 / 修复链路与「注入失败标签」不完全对齐（指标层暴露）
...
| 论文外错误类型 | 归入章节 |
|----------------|----------|
| 缺 `GROQ_API_KEY` 导致 BabyAGI 两例无法执行 | **§二** |
| 替换数据集 / 子集规模 / BOM 等数据工程差异 | **§三** |
| Mock 检测召回不足、真实批混合成功与未跑通应用 | **§四** |
